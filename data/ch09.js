...
Wildcards and type parameters differ. \`<T> void copy(List<T> a, List<T> b)\) requires the same inferred T, while \`void copy(List<? extends T> source, List<? super T> dest)\) supports a producer and consumer relationship. \`List<Object>\) is not a supertype of \`List<String>\); \`List<?>\) is the common read-only view.`,
  ...
  gotchas: [
    ...
    { title: 'Array covariance', md: 'Object[] can reference String[], but a bad store can throw ArrayStoreException.' },
    ...
    { title: 'Array backing view', md: 'Changes through Arrays.asList.set are visible in the original array.' },
    ...
    { title: 'Map merge null', md: 'A merge remapping result of null removes the key.' },
    ...
    { title: 'Queue safe methods', md: 'offer/poll/peek return status or null; add/remove/element throw on failure.' },
